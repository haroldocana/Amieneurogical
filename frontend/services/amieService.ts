...
// ------------------------------------------------------------------
// MOTOR PRINCIPAL DE INFERENCIA CLÍNICA AMIE (GEMINI / VERTEX)
...
export async function runAmieClinicalAnalysis(
  ...
  try {
    ...
    console.warn('Cloud Run API no disponible, ejecutando Gemini vía SDK/Proxy:', backendError);
  }
...
Eres AMIE (Articulate Medical Intelligence Explorer), un copiloto de psiquiatría y neurología médica de precisión clínica operando con Gemini.
  ...
  if (GEMINI_API_KEY) {
    try {
      ...
      const promptParts: any[] = [
        ...
        `EXPEDIENTE PARA TRIANGULACIÓN BIOCLÍNICA:\n${JSON.stringify(patient, null, 2)}`
      ];
      ...
      console.warn('Fallo en SDK oficial Gemini API:', directErr);
    }
  }
...
export async function askAmieAssistant(
  ...
  const systemContext = `
Eres AMIE, Copiloto Clínico Psiquiátrico operando con Gemini.
...
`;
  ...
  if (GEMINI_API_KEY) {
    try {
      ...
      console.warn('Fallo en SDK Gemini en asistente:', directErr);
    }
  }
